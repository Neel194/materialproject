...
router.get(
  "/admin",
  verifyAdmin,
  CacheService.cacheMiddleware(CACHE_TTL.MATERIALS),
  getAllMaterials
);
router.get(
  "/admin/stats",
  verifyAdmin,
  CacheService.cacheMiddleware(CACHE_TTL.STATS),
  getMaterialStats
);
...
router.get(
  "/:id",
  CacheService.cacheMiddleware(CACHE_TTL.SINGLE),
  async (req, res, next) => {
    try {
...
/**
 * Get notifications for a user (by uploadedBy name)
 */
router.get("/notifications/:user", async (req, res, next) => {
  try {
